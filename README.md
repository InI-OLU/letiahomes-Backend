...
A shortlet rental booking platform API built with .NET Core, designed around Clean Architecture principles with a focus on correctness under concurrency — race-safe booking state transitions, not just CRUD.
...
For the deeper architectural reasoning, trade-offs, and decisions made (and reversed) along the way, see [TRADEOFFS.md](./TRADEOFFS.md).
...
- **Unit tested core booking logic** — `CreateBookingCommandHandler` is covered end-to-end across every branch (validation, authorization, conflict detection, success path) using mocked dependencies
...
tests/
  letiahomes.Application.Tests → Unit tests for command/query handlers and background jobs
```
...
```bash
dotnet test tests/letiahomes.Application.Tests
```

Unit tests cover `CreateBookingCommandHandler` across every validation branch, authorization check, conflict-detection path, and the success path — built with mocked repositories (Moq + MockQueryable), no database required. See [TRADEOFFS.md](./TRADEOFFS.md) for the reasoning behind what's covered and what's intentionally left for integration tests.
